...
class WildlifeTracker {
    ...
    initMap() {
        ...
        // Use OpenStreetMap tiles (works offline if cached)
        ...
        filtered.forEach(obs => {
            const marker = L.marker([obs.latitude, obs.longitude])
                ...
                .bindPopup(`
                    ...
                    ${obs.date}<br>
                    ${obs.notes ? obs.notes + '<br>' : ''}
                    <small>${obs.latitude.toFixed(6)}, ${obs.longitude.toFixed(6)}</small>
                `);
